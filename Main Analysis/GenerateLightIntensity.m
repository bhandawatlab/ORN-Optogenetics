...
dirinfo = dir(meta.folderData);
subdirinfo = cell(1,length(dirinfo)-2);
for K = 1 : length(subdirinfo)
    thisdir = dirinfo(K+2).name;
    subdirinfo{K} = dir([meta.folderData '/' thisdir]);
end

% Converting position data to Voltage traces
[p,~] = Voltage_to_Intensity_Converter(meta.calibrationFolder,meta.calibrationFile);
convIV = @(I) (round((I-p(2))/p(1),3)>p(3)).*round((I-p(2))/p(1),3);
convVI = @(V) max(round(p(1).*V+p(2),3),0);
save(meta.Intensity2VoltageFile,'Intensity_spaceN','xN','convIV','convVI','p','-v7.3')

for i = 1:length(subdirinfo)
    close all
    filename = [meta.folderData '\' subdirinfo{i}.name];
    load(filename,'Data');
    ...
    if ~isempty(V2)
        ...
        matfile = [meta.foldStim '\' C{1} '_Stim_Train.mat'];
        save(matfile, '-v7.3','I','I2','V','V2','convIV','convVI','p','fs');
    end
    if isempty(V2)
        matfile = [meta.foldStim '\Bad_Data\' C{1} '_Stim_Train.mat'];
        save(matfile, '-v7.3','I','I2','V','V2','convIV','convVI','p','fs');
    end
end
