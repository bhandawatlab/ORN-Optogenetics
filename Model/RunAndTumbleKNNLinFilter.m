function [synthFlys,state,nTurn,spdAll,curvAll,phiAll,dSpk,allAng,lambda_CW,...
    lambda_Stop,dfSmooth,fSmooth,tt_baseline] = RunAndTumbleKNNLinFilter(self,params)
% get basic parameters
originalFs = self.fs;
fs = params.LLFfs;
len = ceil(params.len*params.LLFfs/params.fs);
...
tt_baseline = repmat({-1.*ones(totFly,len)},3,1);
...
for i = 1:len-1
    ...
    if i>1
        ...
        for c = 1:3
            tt_baseline{c}(:,i) = tt_baseline{c}(:,i-1);
        end
    end
    ...
    if i>1
        ...
        tSinceOff(OffState(:,i)==1,i) = delay;%offset = -10
        tSinceOn(OffState(:,i)==1,i-1) = inf;% switch to on filter, -1 is necessary for it to work properly
        
        OnState(:,i) = dfSmooth(:,i)>15;%BC.Thresh(1);        
        tSinceOn(:,i) = tSinceOn(:,i-1)+1;
        tSinceOn(OnState(:,i)==1,i) = delay;
        tSinceOff(OnState(:,i)==1,i) = inf;% switch to off filter
...
%         %----------------------------------------------------
        ...
        %----------------------------------------------------
        % leaving
        leave = tSinceOff(:,i)<dur.*fs & tSinceOff(:,i)>=0;
        OffRate = getFilteredResponse(currSpk(:,max(1,i-201):i),totFly,self.model.border.leave.ST_trans);
        r = rand(totFly,1)<(OffRate(:,end)./ratio);%./ratio%.*30./100
        newTrack(r & CW_States & leave) = true;
        OffSpd = getFilteredResponse(currSpk(:,max(1,i-201):i),totFly,self.model.border.leave.speed);%mm/s
        OffCurv = getFilteredResponse(currSpk(:,max(1,i-201):i),totFly,self.model.border.leave.curve);%radians
        currSpd(CW_States & leave) = OffSpd(CW_States & leave,end)./fs./self.rBound./10;
        currCurv(CW_States & leave) = OffCurv(CW_States & leave,end).*180./pi;

        
        % entering
%         enter = tSinceOn(:,i)<dur.*fs & tSinceOn(:,i)>=0;
%         OnRate = getFilteredResponse(currSpk(:,max(1,i-201):i),totFly,self.model.border.enter.ST_trans);
%         r = rand(totFly,1)<(OnRate(:,end)./ratio);%./ratio%.*30./100
%         newTrack(r & CW_States & enter) = true;
%         OnSpd = getFilteredResponse(currSpk(:,max(1,i-201):i),totFly,self.model.border.enter.speed);%mm/s
%         OnCurv = getFilteredResponse(currSpk(:,max(1,i-201):i),totFly,self.model.border.enter.curve);%radians
%         currSpd(CW_States & enter) = OnSpd(CW_States & enter,end)./fs./self.rBound./10;
%         currCurv(CW_States & enter) = OnCurv(CW_States & enter,end).*180./pi;
        %----------------------------------------------------
        
        
        [state(newTrack,i)] = getNewState(state(newTrack,i-1),TP,before(newTrack),f(newTrack),df(newTrack),fBaseline);
    ...
    isBaseline = abs(dSpk(:,i))==0 & abs(abs(currSpk(:,i))-fBaseline)<0.001 & t_relFE(:,i)>0;%<1,<0.001
    ttBase(~isBaseline,1) = -1;
    %----------------------------------------------------------------------
...
%             durBaseline = cc & abs(df)<0.01 & abs(f-fBaseline)<0.01 & t_relFE(:,i)>0;% no change in frequency, frequency at baseline, and after first entry            
            %isBaseline = abs(dSpk(:,i))==0 & abs(abs(currSpk(:,i))-fBaseline)<0.001 & t_relFE(:,i)>0;% no change in frequency, frequency at baseline, and after first entry            
            durBaseline = cc & isBaseline;
            tt_baseline{c}(:,i) = ttBase;
            %tt_baseline{c}(~durBaseline,i) = -1;
            if i>1
%                 if any(tt_baseline{c}(durBaseline,i))
%                     a = 1;
%                 end
                tt_baseline{c}(durBaseline,i) = tt_baseline{c}(durBaseline,i-1)+1;% increase baseline count by 1
            end
            ttBase = tt_baseline{c}(:,i);%ttBase(~isBaseline) = -1;
    ...
    if any(cases(:,9))
        ...
        %------------------------------------------------------------------
        % sample from leaving distribution
        tmpCurv = random(self.model.boundary.leaving.avgCurv,sum(c),1);
        while any(tmpCurv<0) %| tmpCurv>5)%any(tmpCurv<0 | tmpCurv>2)
            tmpCurv = random(self.model.boundary.leaving.avgCurv,sum(c),1);
        end
        tmpDur = random(self.model.boundary.leaving.dur,sum(c),1);
        while any(tmpDur<0)% | tmpDur>1.5) 
            tmpDur = random(self.model.boundary.leaving.dur,sum(c),1);
        end
        tmpSpd = random(self.model.boundary.leaving.spd,sum(c),1);
        while any(tmpSpd<0)
            tmpSpd = random(self.model.boundary.leaving.spd,sum(c),1);
        end
        
        currCurv(c) = sign(randi(2,sum(c),1)-1.5).*mod(tmpCurv./fs,360);
        oldDur(c) = round((tmpDur).*fs);
        currDur(c) = oldDur(c);%-1
        currSpd(c) = tmpSpd./fs./self.rBound./10;%currSpd(cc) = currSpd(cc)./fs./self.rBound./10;
        [spdAll(c,i),curvAll(c,i),phiAll(c,i)] = updateGlobalKin(currSpd,currCurv,nanVec,c);
...
% tmp = nan(totFly,18000);
% for i = 1:totFly
%     tmp(i,1:(len-firstEntry(i)+1)) = synthFlys.r(i,firstEntry(i):end);
% end
