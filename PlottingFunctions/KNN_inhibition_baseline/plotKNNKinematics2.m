...
for gen = 1:nGen
    ...
    for i = 1:numel(state2Cons)
        ...
        if strcmpi(m.state.kin,'spd')
            ylab = 'mm/s';ylims = [0 10];ztick = [0,5,10];
        elseif strcmpi(m.state.kin,'totCurv')
            ylab = 'deg';ylims = [0 90];ztick = [0:30:90];
        elseif strcmpi(m.state.kin,'avgCurv')
            ylab = 'deg/s';ylims = [0 20];ztick = [0:10:20];
        elseif strcmpi(m.state.kin,'dur') && strcmpi(m.state.state,'stops')
            ylab = 's';ylims = [0 1.75];ztick = [0:0.5:1.75];
        elseif strcmpi(m.state.kin,'dur') && ~strcmpi(m.state.state,'stops')
            ylab = 's';ylims = [0 1];ztick = [0:0.5:1];
        end
...
switch plotType
    case 'imagesc'
        imagesc(XX(:,1)',YY(1,:),exp(m.val(:,:,tt))',[ylims(1) ylims(2)]);set(gca,'YDir','normal');
        colormap([0 0 0;jet]);
        %plotCB(m,ztick)
    case 'surf'
        surf(XX,YY,exp(m.val(:,:,tt)));view([45 45]);hold on;
        points=[[min(XX(:)) min(XX(:)) max(XX(:)) max(XX(:))]' ...
            [0 max(YY(:)) max(YY(:)) 0]' exp(m.baseline(1)).*[1 1 1 1]'];
...
[newTicks,ndx] = sort([cb.Ticks, exp(m.baseline(1))]);
